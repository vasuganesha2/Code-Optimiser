...
# Pass sequence to cycle through repeatedly
...
def run_task(task):
    ...
    pass_index = 0          # where we are in PASS_SEQUENCE
    no_progress_streak = 0  # consecutive useless actions

    for _ in range(env.max_steps):
        # If both passes in a full cycle did nothing, we're done — stop early
        ...
        action = Action(pass_name=action_name)
        obs, reward, done, info =   env.step(action)
        total_reward += reward

        if info["useful"]:
            #Progress made — reset streak and restart cycle from fold
            no_progress_streak = 0
            pass_index = 0
        else:
            #This pass did nothing — try the next one
