...
from env.passes import (
    ...
    local_cse,
    global_cse,
    lazy_code_motion,   # Algorithm 9.36 — accepts / returns IRProgram
    store_load_fwd,
    dead_store_elim,
)
from env.ops import (
...
AVAILABLE_PASSES = {
    ...
    "dead_store_elim": dead_store_elim,
    "stop":            noop,
}
...
class CompilerEnv:
    ...
    def _build_cfg(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "edges": edges,
            "exit": [i for i in range(n) if not edges[i]],
        }
    ...
    def _live_at_exit(self) -> List[Set[str]]:
        ...
        # Seed: the last instruction that produces a value is the program output.
        final_out: str = ""
    ...
    def _can_apply(self, name: str) -> bool:
        ...
        # BUG FIX #6: global_cse was never handled, so _can_apply always
        # returned False for it and the pass was silently skipped every time.
        # global_cse = local_cse (upgraded), so the applicability check is
        # identical: look for any repeated math expression.
        if name == "global_cse":
            seen = set()
            for inst in self.program:
                if is_math(inst.op):
        ...
        if name == "const_fold":
            ...
            for inst in self.program:
                if is_math(inst.op) and inst.op != "const":
                    if all(
                        (isinstance(a, int) or a in consts)
                        for a in inst.args
                    ):
                        return True
            return False
        ...
        if name == "store_load_fwd":
            stores = {
                inst.args[1]
                for inst in self.program
                if inst.op == "store" and len(inst.args) == 2
            }
            loads = {
                inst.args[0]
                for inst in self.program
            ...
            return not stores.isdisjoint(loads)

        if name == "dead_store_elim":
            store_ptrs = [
                inst.args[1]
                for inst in self.program
                if inst.op == "store" and len(inst.args) == 2
            ]
            return len(store_ptrs) != len(set(store_ptrs))
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, Dict]:
        ...
        if action.pass_name == "stop":
            rem_opts = any(
                self._can_apply(p)
                for p in [
                    "const_fold", "dead_code_elim", "code_motion",
                    "copy_prop", "local_cse", "lcm",
                ]
            )
